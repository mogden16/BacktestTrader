...
TRAIL_STOP_PERCENTAGE = float(os.getenv('TRAIL_STOP_PERCENTAGE'))
RUNNER_FACTOR = float(os.getenv('runnerFactor'))


class OrderBuilder:

    def __init__(self):
        self.isTesting = os.getenv('isTesting')=="True"
        ...
        if not self.isTesting:
            ...
            try:
                ...
                resp = self.tdameritrade.getQuote(
                    symbol if asset_type == "EQUITY" else pre_symbol)
                # print('resp',resp)
                ...
                if list(resp.keys())[0] == "error":
                    print(f'error scanning for {symbol}') if asset_type == "EQUITY" else (f'error scanning for {pre_symbol}')
                    # print(resp)
                    self.error += 1
                    return
